"""Aggregate SIMM margins by risk and product class."""
...
class SIMM:
    """Calculate SIMM initial margin results for a CRIF dataset."""
    ...
    def simm_risk_class(self, crif: pd.DataFrame) -> Dict[str, Dict[str, float]]:
        """Return SIMM margin by risk class for the provided CRIF subset."""
        margin = MarginByRiskClass(crif, self.calc_currency)
        df_margin_aggregated = (
            margin.IRDeltaMargin()
            + margin.DeltaMargin()
            + margin.IRVegaMargin()
            + margin.VegaMargin()
        ...
        dict_margins: Dict[str, Dict[str, float]] = df_margin_aggregated.to_dict()
        ...
        logger.debug("Risk class margins computed: %s", dict_margins)
        return dict_margins

    # SIMM by product class
    def simm_product(self, product_class: str) -> float:
        """Return SIMM margin for a single product class."""
        crif = self.crif[(self.crif["ProductClass"] == product_class)]

        dict_simm_risk_class: Dict[str, float] = {}
        ...
        for i in range(6):
            for j in range(6):
                psi = 1 if i == j else wnc.psi(risk_class_list[i], risk_class_list[j])
    ...
    def results_product_class(self, product_class: str) -> pd.DataFrame:
        """Return a pivot table of SIMM results for a product class."""
        crif = self.crif[(self.crif["ProductClass"] == product_class)]
        ...
        for risk_class in dict_results:
            ...
            df_local_sensiType = pd.DataFrame(
                dic_sensiType.items(), columns=["Risk Measure", "SIMM_RiskMeasure"]
            )
            ...
            df_local_riskType = pd.DataFrame(
                {"Risk Class": [risk_class], "SIMM_RiskClass": [IM_risk_class]}
            )
        ...
        df_outerJoin = pd.merge(
            df_risk_class, df_main, left_on="Risk Class", right_on="Risk Class", how="outer"
        )
        df_outerJoin["Product Class"] = product_class

        return pd.pivot_table(
            df_outerJoin, index=["Product Class", "Risk Class", "SIMM_RiskClass", "Risk Measure"]
        )

    def addon_margin(self) -> float:
        """Return additional add-on margin from CRIF parameters."""
        crif_factorNotional = self.crif[
            (self.crif["RiskType"].isin(["Param_AddOnNotionalFactor", "Notional"]))
        ]
        ...
        for qualifier in qualifier_list:
            ...
            crif_factor = crif_qualifier[
                crif_qualifier["RiskType"] == "Param_AddOnNotionalFactor"
            ]
    ...
    def calculate_simm(self) -> pd.DataFrame:
        """Calculate SIMM totals and populate the breakdown table."""
        addon_ms = 0.0  # addon multiplicative scales
        dict_addon = {}
        df_total = pd.DataFrame()

        for product_class in utils.product_list(self.crif):
            logger.info("Calculating SIMM for product class: %s", product_class)
        ...
        logger.info("SIMM calculation completed. Total: %s", self.simm)
        return df
