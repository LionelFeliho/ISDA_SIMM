"""Margin calculations by risk class."""
...
class MarginByRiskClass:
    """Compute margin contributions for each SIMM risk class."""
    ...
    def IRDeltaMargin(self) -> pd.DataFrame:
        ...
        if (
            "Risk_IRCurve" not in self.list_risk_types
            and "Risk_Inflation" not in self.list_risk_types
            and "Risk_XCcyBasis" not in self.list_risk_types
        ):
        ...
        crif = self.crif[
            (self.crif["RiskType"].isin(["Risk_IRCurve", "Risk_Inflation", "Risk_XCcyBasis"]))
        ]
        ...
        for currency in currency_list:
            ...
            crif_wo_xccybasis = crif_currency.drop(
                crif_currency[(crif_currency.RiskType == "Risk_XCcyBasis")].index
            )
            ...
            list_rates_risk_types = utils.unique_list(
                [
                    risk_class
                    for risk_class in crif_currency["RiskType"]
                    if risk_class in ["Risk_IRCurve", "Risk_Inflation", "Risk_XCcyBasis"]
                ]
            )
                ...
                if risk_class == "Risk_Inflation":
                    ...
                    for subcurve in subcurve_list:
                        ...
                        for tenor in utils.tenor_list(crif_subcurve):
                            ...
                            dict_sensitivities_tenor[tenor] = utils.sum_sensitivities(
                                crif_tenor
                            )
        ...
        for i in range(len(currency_list)):
            for j in range(len(currency_list)):
                ...
                g = min(dict_CR[currency_b], dict_CR[currency_c]) / max(
                    dict_CR[currency_b], dict_CR[currency_c]
                )
        ...
        logger.debug("IR Delta margin computed: %s", updates["Rates"]["Delta"])
        return pd.DataFrame(updates)
    ...
    def DeltaMargin(self) -> pd.DataFrame:
        ...
        if (
            ("Risk_FX" not in self.list_risk_types)
            and ("Risk_CreditQ" not in self.list_risk_types)
            and ("Risk_CreditNonQ" not in self.list_risk_types)
            and ("Risk_Equity" not in self.list_risk_types)
            and ("Risk_Commodity" not in self.list_risk_types)
        ):
            return pd.DataFrame(updates)
        ...
        list_risk_classes = [
            risk_class for risk_class in self.list_risk_types if risk_class in allowed_risk_classes
        ]
        for risk_class in list_risk_classes:
            ...
            if risk_class == "Risk_FX":
                ...
                for currency in currency_list:
                    ...
                    if currency == self.calculation_currency:
                        RW = 0
                    elif (is_given_currency is True) and (is_calc_currency is True):
                        RW = wnc.fx_rw["High"]["High"]
                    elif (is_given_currency is True) and (is_calc_currency is False):
                        RW = wnc.fx_rw["High"]["Regular"]
                    elif (is_given_currency is False) and (is_calc_currency is True):
                        RW = wnc.fx_rw["Regular"]["High"]
                    elif (is_given_currency is False) and (is_calc_currency is False):
            ...
            # CreditQ, CreditNonQ, Equity, Commodity
                ...
                for bucket in bucket_list:
                    if bucket == 0:
                        crif_bucket = crif_others[
                            (crif_others["RiskType"] == risk_class)
                            & (crif_others["Bucket"] == "Residual")
                        ]
                    else:
                        crif_bucket = crif_others[
                            (crif_others["RiskType"] == risk_class)
                            & (crif_others["Bucket"].isin([bucket, str(bucket)]))
                        ]
        ...
        logger.debug("Delta margin computed: %s", updates)
        return pd.DataFrame(updates)
    ...
    def IRVegaMargin(self) -> pd.DataFrame:
        ...
        allowed_risk_classes = [
            risk_class
            for risk_class in self.list_risk_types
            if risk_class in ["Risk_IRVol", "Risk_InflationVol"]
        ]
        if (
            "Risk_IRVol" not in self.list_risk_types
            and "Risk_InflationVol" not in self.list_risk_types
        ):
            return pd.DataFrame(updates)
        ...
        for risk_class in allowed_risk_classes:
            ...
            for currency in currency_list:
                crif_currency = self.crif[
                    (self.crif["RiskType"].isin(["Risk_IRVol", "Risk_InflationVol"]))
                    & (self.crif["Qualifier"] == currency)
                ]
                ...
                for b in range(len(currency_list)):
                    for c in range(len(currency_list)):
                        ...
                        g = min(dict_VCR[currency_b], dict_VCR[currency_c]) / max(
                            dict_VCR[currency_b], dict_VCR[currency_c]
                        )
                        gamma = wnc.ir_gamma_diff_ccy
                ...
                logger.debug("IR Vega margin computed: %s", updates["Rates"]["Vega"])
                return pd.DataFrame(updates)
        ...
        for risk_class in list_risk_classes:
            ...
            if risk_class in fx:
                ...
                for currency_pair in utils.currencyPair_list(self.crif):
                    ...
                    crif_fx = self.crif[
                        (self.crif["RiskType"] == risk_class)
                        & (self.crif["Qualifier"].isin(pair_list))
                    ]
                    ...
                    VT = wnc.T(risk_class, "Vega", currency=currency_pair)
            ...
            # Equity, Commodity, Credit
                ...
                for bucket in bucket_list:
                    if bucket == 0:
                        crif_others = self.crif[
                            (self.crif["RiskType"] == risk_class)
                            & (self.crif["Bucket"] == "Residual")
                        ]
                    else:
                        crif_others = self.crif[
                            (self.crif["RiskType"] == risk_class)
                            & (self.crif["Bucket"].isin([bucket, str(bucket)]))
                        ]
        ...
        for risk_class in allowed_risk_classes:
            ...
            for currency in currency_list:
                ...
                list_rates_risk_types = utils.unique_list(
                    [
                        risk_class
                        for risk_class in list(crif_currency["RiskType"])
                        if risk_class in ["Risk_IRVol", "Risk_InflationVol"]
                    ]
                )
                ...
                if (
                    list_rates_risk_types == ["Risk_InflationVol"]
                    and (crif_currency["AmountUSD"].sum() == 0)
                    and (self.calculation_currency == currency)
                ):
                    return pd.DataFrame(updates)
    ...
    def CurvatureMargin(self) -> pd.DataFrame:
        ...
        allowed_risk_classes = [
            risk_class
            for risk_class in self.list_risk_types
            if risk_class in credit + equity + commodity + fx
        ]
        for risk_class in allowed_risk_classes:
            K_Res = 0
            list_K = []
            ...
            # Equity, Commodity, Credit
            if risk_class in credit + equity + commodity:
                bucket_list = utils.bucket_list(self.crif[(self.crif["RiskType"] == risk_class)])
                for bucket in bucket_list:
                    if bucket == 0:
                        crif_risk_class = self.crif[
                            (self.crif["RiskType"] == risk_class)
                            & (self.crif["Bucket"] == "Residual")
                        ]
                    else:
                    ...
                    for qualifier in utils.unique_list(crif_risk_class, "Qualifier"):
                        crif_qualifier = crif_risk_class[
                            crif_risk_class["Qualifier"] == qualifier
                        ]
                        ...
                        if risk_class in equity + commodity:
                            ...
                            list_lable2 = utils.unique_list(crif_qualifier, "Label2")
                            for label2 in list_lable2:
                                crif_label2 = crif_qualifier[
                                    crif_qualifier["Label2"] == label2
                                ]

                                for tenor in utils.tenor_list(crif_qualifier):
                                    crif_tenor = crif_label2[
                                        crif_label2["Label1"] == tenor
                                    ]
            ...
            if (risk_class == "Risk_EquityVol") and (12 in bucket_list) and (len(bucket_list) == 1):
                continue
