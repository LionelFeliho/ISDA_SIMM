"""Aggregate risk sensitivities to SIMM measures."""
...
def k_delta(
    risk_class: str,
    list_WS: Sequence[float],
    list_CR: Sequence[float] | None = None,
    bucket: Sequence | None = None,
    tenor: Sequence[str] | None = None,
    index: Sequence[str] | None = None,
    calculation_currency: str = "USD",
) -> float:
    """Calculate delta aggregation term K for a risk class."""
    K = sum([np.array([WS], dtype="double")[0] ** 2 for WS in list_WS])
            ...
            # Equity, Commodity, FX
                ...
                if risk_class in list_equity + list_commodity:
                    ...
                    if calculation_currency not in wnc.high_vol_currency_group:
                        if (is_ccy1_high_vol is True) and (is_ccy2_high_vol is True):
                            rho = wnc.fx_reg_vol_corr["High"]["High"]
                        elif (is_ccy1_high_vol is True) and (is_ccy2_high_vol is False):
                            rho = wnc.fx_reg_vol_corr["High"]["Regular"]
                        elif (is_ccy1_high_vol is False) and (is_ccy2_high_vol is True):
    ...
    logger.debug("Delta K computed for %s: %s", risk_class, K)
    return math.sqrt(K)
...
def k_vega(
    risk_class: str,
    VR: Sequence[float],
    VCR: Sequence[float] | None = None,
    bucket: Sequence | None = None,
    index: Sequence[str] | str = "",
) -> float:
    """Calculate vega aggregation term K for a risk class."""
    if index == "":
        index = [""] * len(VR)
    ...
    logger.debug("Vega K computed for %s: %s", risk_class, K)
    return math.sqrt(K)


def k_curvature(
    risk_class: str,
    CVR_list: Sequence[float],
    bucket: Sequence | None = None,
    index: Sequence[str] | None = None,
) -> float:
    """Calculate curvature aggregation term K for a risk class."""
    ...
    logger.debug("Curvature K computed for %s: %s", risk_class, K)
    return math.sqrt(K)
