...
and roll sensitivities forward to each tenor bucket provided by the caller.
...
Otherwise, the remaining tenor is bucketed using a prorata temporis rule
between the nearest lower and upper buckets:
\[
T_{\text{rem}} \in [T_{\text{low}}, T_{\text{up}}],\quad
w_{\text{low}} = \frac{T_{\text{up}} - T_{\text{rem}}}{T_{\text{up}} - T_{\text{low}}},\quad
w_{\text{up}} = 1 - w_{\text{low}}.
\]
The sensitivity is split into two rows with weights $w_{\text{low}}$ and
$w_{\text{up}}$ assigned to $T_{\text{low}}$ and $T_{\text{up}}$, respectively.
If $T_{\text{rem}}$ is below the minimum bucket, 100\% is assigned to the
minimum bucket. If $T_{\text{rem}}$ is above the maximum bucket, 100\% is
assigned to the maximum bucket.
